...
import { loginSuccess, authModalOpen, authModalClose, authMe } from "../../store/AuthSlice.tsx";
...
export const Account = () => {
    ...
    const handleCloseModal = () => dispatch(authModalClose())
  ...
  if (meQuery.isError) {
    return (
      <>
        <button className={`header__button`} onClick={handleClickEnter}>
          Войти
        </button>
  ...
  if (meQuery.isSuccess) {
    ...
    dispatch(authMe(meQuery.data));
    dispatch(authModalClose());
    return (
      <Link
        to="/profile"
        className={`header__button ${windowWidth > 600 && pathname === "/profile" ? `active` : ``}`}
      >
        {meQuery.data.name}
      </Link>
    );
